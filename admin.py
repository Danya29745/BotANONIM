...
def admin_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💬 Последние вопросы", callback_data="admin:recent:0")],
...
def recent_nav_kb(offset: int, total: int, page_size: int = 5) -> InlineKeyboardMarkup:
    buttons = []
    nav = []
    if offset > 0:
        nav.append(InlineKeyboardButton(text="⬅️ Назад", callback_data=f"admin:recent:{offset - page_size}"))
    if offset + page_size < total:
        nav.append(InlineKeyboardButton(text="Вперёд ➡️", callback_data=f"admin:recent:{offset + page_size}"))
    if nav:
        buttons.append(nav)
...
async def cb_recent(call: CallbackQuery):
    ...
    total = len(all_qs)
    page = all_qs[offset: offset + PAGE]
    ...
    lines = [f"💬 <b>Лента вопросов</b> [{offset+1}–{min(offset+PAGE, total)} из {total}]\n"]

    for q in page:
        owner_name = q["owner_name"] or "?"
        owner_u = f" (@{q['owner_username']})" if q.get("owner_username") else ""
        asker_name = q["asker_name"] or "Аноним"
        asker_u = f" (@{q['asker_username']})" if q.get("asker_username") else ""
        revealed = " 🔓" if q.get("identity_revealed") else ""

        answered = "✅ Отвечено" if q["answer_text"] else "⏳ Без ответа"
        q_short = q["question_text"][:100] + ("..." if len(q["question_text"]) > 100 else "")
        a_short = ""
        if q["answer_text"]:
            a_short = "\n↩️ <i>" + q["answer_text"][:80] + ("..." if len(q["answer_text"]) > 80 else "") + "</i>"

        lines.append(
            f"━━━━━━━━━━━━━━━━\n"
            f"<b>#{q['id']}</b> {answered}\n"
            f"👤 Владелец: <b>{owner_name}</b>{owner_u}\n"
            f"🕵️ Автор{revealed}: <b>{asker_name}</b>{asker_u}\n"
            f"❓ {q_short}{a_short}\n"
            f"🕒 {q['asked_at'][:16]}"
        )
    ...
    await call.message.edit_text(
        "\n".join(lines),
        parse_mode="HTML",
        reply_markup=recent_nav_kb(offset, total, PAGE)
    )
